...
public class DocumentSwitch<T> extends Switch<T> {
	...
	protected T doSwitch(int classifierID, EObject theEObject) {
		...
		case DocumentPackage.BPM_HUMAN_TASK_DOCUMENT: {
			...
			if (result == null)
				result = caseLuniferaDocDocument(bpmHumanTaskDocument);
			if (result == null)
				result = caseNamedDocument(bpmHumanTaskDocument);
			if (result == null)
				result = defaultCase(theEObject);
			return result;
		}
	...
	/**
	 * Returns the result of interpreting the object as an instance of '<em>UI Description</em>'. <!-- begin-user-doc
	 * --> This implementation returns null; returning a non-null result will terminate the switch. <!-- end-user-doc
	 * -->
	 * 
	 * @param object

...
public class RichStringAwareSourceViewer extends XtextSourceViewer {
	...
	private static Logger log = Logger
			.getLogger(RichStringAwareSourceViewer.class);
	...
	public static class Factory extends DefaultFactory {
		...
		public XtextSourceViewer createSourceViewer(Composite parent,
			...
			RichStringAwareSourceViewer result = new RichStringAwareSourceViewer(
					parent, ruler, overviewRuler, showsAnnotationOverview,
					styles);
	...
	public RichStringAwareSourceViewer(Composite parent, IVerticalRuler ruler,
	...
	protected void shift(boolean useDefaultPrefixes, boolean right,
		...
		if (useDefaultPrefixes && ignoreWhitespace) { // let's assume we toggled
														// comments
			...
			try {
				...
				ITypedRegion[] regions = TextUtilities.computePartitioning(d,
						getDocumentPartitioning(), block.getOffset(),
						block.getLength(), false);
				regions = merger.merge(regions);
				int lineCount = 0;
				int[] lines = new int[regions.length * 2]; // [start line, end
															// line, start line,
															// end line, ...]
				...
				@SuppressWarnings("rawtypes")
				Map map = (useDefaultPrefixes ? fDefaultPrefixChars
						: fIndentChars);
				for (int i = 0, j = 0; i < regions.length; i++, j += 2) {
					String[] prefixes = (String[]) copiedSelectContentTypePlugin(
							regions[i].getType(), map);
					if (prefixes != null && prefixes.length > 0
							&& lines[j] >= 0 && lines[j + 1] >= 0) {
						if (right)
							copiedShiftRight(lines[j], lines[j + 1],
									prefixes[0]);
						else
							copiedShiftLeft(lines[j], lines[j + 1], prefixes,
									ignoreWhitespace);
					}
				}
	...
	private void copiedShiftLeft(int startLine, int endLine, String[] prefixes,
			boolean ignoreWhitespace) {
		...
		try {
			...
			for (int i = 0; i < occurrences.length; i++) {
				...
				if (index > -1) {
					// remember where prefix is in line, so that it can be
					// removed
					int length = prefixes[found[1]].length();
					if (length == 0 && !ignoreWhitespace
							&& line.getLength() > 0) {
	...
	private Object copiedSelectContentTypePlugin(String type,
			@SuppressWarnings("rawtypes") Map plugins) {

 ...
 *
 * Work based org.eclipse.xtend.core.typing.XtendTypeProvider
 *******************************************************************************/
...
public class LuniferaDocTypeProvider extends XbaseWithAnnotationsTypeProvider {
	...
	protected JvmTypeReference type(XExpression expression,
		...
		if (expression instanceof RichString) {
			...
			return _type((RichStringForLoop) expression, rawExpectation,
					rawType);
			...
			return _type((RichStringLiteral) expression, rawExpectation,
					rawType);
		} else {
	...
	@Override
	protected JvmTypeReference expectedType(EObject container,
			EReference reference, int index, boolean rawType) {
		if (container instanceof RichStringElseIf) {
			return _expectedType((RichStringElseIf) container, reference,
					index, rawType);
		} else if (container instanceof RichStringForLoop) {
			return _expectedType((RichStringForLoop) container, reference,
					index, rawType);
		} else if (container instanceof RichStringIf) {
			return _expectedType((RichStringIf) container, reference, index,
					rawType);
		} else {
	...
	protected JvmTypeReference _type(RichString richString,
			JvmTypeReference rawExpectation, boolean rawType) {
		JvmTypeReference stringType = getTypeReferences().getTypeForName(
				String.class, richString);
		if (getTypeReferences().is(getExpectedType(richString, rawType),
				String.class))
			return stringType;
		return getTypeReferences().getTypeForName(CharSequence.class,
				richString);
	}
	...
	protected JvmTypeReference _expectedType(RichStringIf container,
		...
		return getTypeReferences()
				.getTypeForName(CharSequence.class, container);
	}

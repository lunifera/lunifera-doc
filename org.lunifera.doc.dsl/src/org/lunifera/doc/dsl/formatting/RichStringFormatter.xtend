		...
		for (line : lines) {
			if (impl.model.rootIndentLenght > 0) {
				...
				if (line.leadingSemanticNewLine)
					doc += new NewLineData(nloffset, nllength, increaseIndentationChange, decraseIndentationChange,
				...
				if (!line.chunks.empty) {
					...
					var length = line.indentLength - impl.model.rootIndentLenght
					if (length < 0) {
						length = 0
					}
...
class RichStringToLineModel extends AbstractRichStringPartAcceptor.ForLoopOnce {
	...
	override void acceptOrderedListStart(RichStringOrderedList object) {
		super.acceptOrderedListStart(object)
		startContent()
		indentNextLine = true
	}

	override void acceptOrderedListEnd() {
		super.acceptOrderedListEnd()
		outdentThisLine()
		startContent()
	}

	override void acceptListStart(RichStringList object) {
		super.acceptListStart(object)
		startContent()
		indentNextLine = true
	}

	override void acceptListEnd() {
		super.acceptListEnd()
		outdentThisLine()
		startContent()
	}

	override void acceptTableStart(RichStringTable object) {
		super.acceptTableStart(object)
		startContent()
		indentNextLine = true
	}

	override void acceptTableEnd() {
		super.acceptTableEnd()
		outdentThisLine()
		startContent()
	}

	override void acceptTableRowStart(RichStringTableRow object) {
		super.acceptTableRowStart(object)
		startContent()
		indentNextLine = true
	}

	override void acceptTableRowEnd() {
		super.acceptTableRowEnd()
		outdentThisLine()
		startContent()
	}

	override void acceptTableCellStart(RichStringTableCell object) {
		super.acceptTableCellStart(object)
		startContent()
		indentNextLine = true
	}

	override void acceptTableCellEnd() {
